...
function isRenderer<P>(
  renderer: React.ReactNode | Renderer<P>
): renderer is Renderer<P> {
  return isFunction(renderer);
}

function normalizeRenderer<P>(
  potential: React.ReactNode | Renderer<P>
): Renderer<P> {
...
export type { Renderable, Renderer };
