...
const Arrow = React.forwardRef<Polyline, Props>((props, ref) => {
  ...
  const vectorA = mapLib.rotate(
    unitVector,
    mapLib.degreesToRads(props.arrowAngle)
  );
  const vectorB = mapLib.rotate(
    unitVector,
    mapLib.degreesToRads(-props.arrowAngle)
  );
  ...
  return (
    <React.Fragment>
      <Polyline {...props} ref={ref} />
      <Polyline {...props} positions={lineA} />
      <Polyline {...props} positions={lineB} />
    </React.Fragment>
  );
});
Arrow.defaultProps = {
