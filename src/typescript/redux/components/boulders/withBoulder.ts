...
const query = selectNormalizr(BoulderSchema, {
  area: { crag: "empty", polygon: true },
  routes: "empty",
  commentable: true,
});

const selectProps = (state: State, props: OwnProps) => props.boulderId;
const selectBoulder = (entities, boulderId) =>
  denormalize(boulderId, query, entities);
...
const mapDispatchToProps = (dispatch, ownProps: OwnProps) => {
  return {
    fetch: () => dispatch(fetchBoulder("singleton-fetch")(ownProps.boulderId)),
  };
};

type StateProps = ReturnType<typeof mapStateToProps>;
type DispatchProps = ReturnType<typeof mapDispatchToProps>;
const withBoulder = asyncComponent<StateProps, DispatchProps, OwnProps>(
  mapStateToProps,
  mapDispatchToProps,
  (props) =>
    !!(
